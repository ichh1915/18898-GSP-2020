...
def draw_degree_hist(G,n,p):
    ...
    file_name = 'Q8_Assets/Degree_SmallWorld_N_'+str(n)+'_P_'+str(p)+'.pdf'
    plt.savefig(file_name,bbox_inches="tight")
    plt.close(fig)
    del fig
...
def graph_properties(G,n,p):
    ...
    np.savetxt('Q8_Assets/Adjacency_N_'+str(n)+'_P_'+str(p)+'.csv', A, delimiter=",")
    np.savetxt('Q8_Assets/Laplacian_N_'+str(n)+'_P_'+str(p)+'.csv', L, delimiter=",")
    np.savetxt('Q8_Assets/Betweeness_N_'+str(n)+'_P_'+str(p)+'.csv', betwn_ctrlity_array, delimiter=",")
    print('Clustering = ',c, '\n')
    print('Diameter = ',d, '\n')
...
if __name__ == '__main__':
    ...
    for n in [200, 1000]:
        for p in [.5/n, 5/n]:
                print('\nSmall World Graph with: n=',n, ', p=',p,'\n')
                SmallWorld(n,p)
